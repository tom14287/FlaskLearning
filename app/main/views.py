...
def index_view():
	...
	if(g.user != None and g.user.is_authenticated):
		return render_template('index.html', Type=g.user.UserType)
...
def search_link(page_num=1, Pane=0, Command=''):
	...
	return render_template('search.html', Pane=Pane,keyword=get_keyword,\
						   ...
						   lowbound=get_lowbound,upperbound=get_upperbound, Command=Command,sort_type=sort_type, Type=login_type)
