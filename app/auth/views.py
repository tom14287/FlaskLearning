...
@auth_.route('/login&test=<test>', methods=['GET', 'POST'])
def login(test=0):
	form = RegisterForm(request.form)
	test = int(test)
		...
		if request.form['action'] == 'login':
			...
			if form.validate_on_submit():
				...
				if ret == 'NOACCOUNT' or ret == 'WRONGPWD':
					...
					return render_template('login.html', form=form, page='login',test=test)
				else:
					return redirect(url_for('main.index_view'))
		elif request.form['action'] == 'register':
			if form.validate_on_submit():
				print "reg"
				ret = register_func(form.username.data, form.email.data, form.password.data, form.confirm.data,
										form.type.data)
				if ret == "SUCCEED":
					return redirect(url_for("auth.unconfirmed"))

			return render_template('login.html', form=form, page='register', test=test)
...
def update_user(id, name, email, tel, pwd):
	...
	db.session.execute("update User set UserPassword='%s',UserName='%s',"
					   "UserEmail='%s',UserTEL='%s' where UserID=%d" % (user.UserPassword,
																		user.UserName, user.UserEmail, user.UserTEL,
																		user.UserID)).all()
	db.session.commit()
